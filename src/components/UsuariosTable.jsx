...
/**
 * Componente para exibir uma tabela de utilizadores do sistema.
 * @param {Object} props
 * @param {Array} props.usuarios - A lista de utilizadores a ser exibida.
 * @param {Function} props.onEdit - Função para editar o utilizador.
 * @param {Function} props.onDelete - Função para excluir o utilizador.
 */
    ...
    return (
      <div className="text-center p-10 bg-white rounded-lg shadow">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Pode começar por adicionar um novo utilizador.
        </p>
      </div>
    );
  }
    ...
    <>
      <div className="md:hidden space-y-4">
        {usuarios.map((usuario) => (
          <div key={usuario.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-2 relative">
            <div className="flex justify-between items-start">
              <div className="flex flex-col">
                <div className="text-base font-bold text-gray-900">{usuario.nome}</div>
            ...
            <div className="text-sm text-gray-600 mt-1">
              <span className="font-medium text-gray-700 mr-2">Perfil:</span>
              {usuario.perfil?.nome || <span className="text-gray-400">Sem Perfil</span>}
            </div>
      ...
      <div className="hidden md:block overflow-x-auto premium-card bg-white rounded-3xl shadow-sm border border-gray-100">
        <table className="min-w-full divide-y divide-gray-100">
          ...
          <tbody className="bg-white divide-y divide-gray-50">
            {usuarios.map((usuario) => (
              <tr key={usuario.id} className="hover:bg-ibdn-bg/30 transition-colors group">
